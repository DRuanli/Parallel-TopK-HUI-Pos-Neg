 ...
 *
 * VARIANT 5: Exponential Decay Split
 * Uses depth-adaptive split ratios for optimal parallelism
 */
public class variant5 {
    ...
    // ============================================================================
    // FORKJOIN TASKS FOR PARALLEL PROCESSING (EXPONENTIAL DECAY SPLIT)
    // ============================================================================

    /**
     * Parallel task for processing single-item prefixes
     * VARIANT 5: Uses depth-adaptive split ratios
     */
    private class PrefixMiningTask extends RecursiveAction {
        ...
        private final int depth;  // Recursion depth
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // EXPONENTIAL DECAY: Choose split strategy based on depth
                int mid = findDepthAdaptiveSplitForItems(start, end, depth);
                PrefixMiningTask left = new PrefixMiningTask(sortedItems, singleItemLists, start, mid, depth + 1);
                PrefixMiningTask right = new PrefixMiningTask(sortedItems, singleItemLists, mid, end, depth + 1);
        ...
        /**
         * Finds split point using depth-adaptive strategy:
         * - Depth 0-2: Balanced 50/50 split (maximize top-level parallelism)
         * - Depth 3-5: Asymmetric 33/67 split (enable work-stealing)
         * - Depth 6+: Simple midpoint (minimize overhead)
         * ENSURES: start < result < end (strictly between)
         */
        private int findDepthAdaptiveSplitForItems(int start, int end, int depth) {
            int size = end - start;

            // Deep recursion: use simple midpoint to minimize overhead
            if (depth >= 6) {
                return start + (size / 2);
            }

            // Calculate total weight for range [start, end)
            ...
            // Choose target weight based on depth
            double targetWeight;
            if (depth <= 2) {
                // Depth 0-2: Balanced 50/50 split
                targetWeight = totalWeight / 2.0;
            } else {
                // Depth 3-5: Asymmetric 33/67 split
                targetWeight = totalWeight / 3.0;
            }

            // Find split point at target weight
    ...
    private class ExtensionSearchTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // EXPONENTIAL DECAY: Choose split strategy based on depth
                int mid = findDepthAdaptiveSplitForExtensions(start, end, depth);
                ExtensionSearchTask left = new ExtensionSearchTask(
                    prefix, extensions, singleItemLists, start, mid, depth + 1
                );
                ExtensionSearchTask right = new ExtensionSearchTask(
                    prefix, extensions, singleItemLists, mid, end, depth + 1
                );
         ...
         * - Depth 0-2: Balanced 50/50 split
         * - Depth 3-5: Asymmetric 33/67 split
         * - Depth 6+: Simple midpoint
         * ENSURES: start < result < end (strictly between)
         */
        private int findDepthAdaptiveSplitForExtensions(int start, int end, int depth) {
            int size = end - start;
        ...
        private void processExtension(int index) {
            ...
            UPUList joined = variant5.this.join(prefix, extension);
    ...
    public variant5(Map<Integer, Double> itemProfits, int k, double minPro, boolean debug, boolean debugVerbose) {
    ...
    public List<Itemset> mine(List<Transaction> rawDatabase) {
        if (DEBUG) {
            System.err.println("[DEBUG] ===== PTK-HUIM-U± Algorithm Started (VARIANT 5: Exponential Decay) =====");
            System.err.println("[DEBUG] Task Splitting: Depth-Adaptive (50/50 @ depth 0-2, 33/67 @ depth 3-5, midpoint @ depth 6+)");
        ...
        PrefixMiningTask rootTask = new PrefixMiningTask(
            sortedItems, singleItemLists, 0, sortedItems.size(), 0  // depth starts at 0
        );
    ...
    private void searchEnhanced(UPUList prefix, List<UPUList> extensions,
        ...
        ExtensionSearchTask task = new ExtensionSearchTask(
            prefix, extensions, singleItemLists, 0, extensions.size(), 0  // depth starts at 0
        );
        task.invoke();
    }
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 4 || args.length > 5) {
            System.err.println("Usage: variant5 <database_file> <profit_file> <k> <min_probability> [--debug | --debug-verbose]");
            System.exit(1);
        }
        ...
        variant5 algorithm = new variant5(profits, k, minPro, debug, debugVerbose);
