 ...
 *
 * VARIANT 4: Top-Heavy First Split
 * Isolates heaviest items for priority parallel processing
 */
public class variant4 {
    ...
    // ============================================================================
    // FORKJOIN TASKS FOR PARALLEL PROCESSING (TOP-HEAVY FIRST SPLIT)
    // ============================================================================

    /**
     * Parallel task for processing single-item prefixes
     * VARIANT 4: Isolates top-heavy items (high PTWU) for priority processing
     */
    private class PrefixMiningTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // TOP-HEAVY FIRST: Split at 75% weight to isolate heavy items
                int mid = findTopHeavySplitForItems(start, end);
        ...
        /**
         * Finds split point that isolates top-heavy items (75% weight point)
         * Items are sorted ascending by PTWU, so heavy items are at the end
         * Splitting at 75% gives lighter items to left, heavier 25% to right
         * ENSURES: start < result < end (strictly between)
         */
        private int findTopHeavySplitForItems(int start, int end) {
            ...
            // Step 2: Find split point at 75% of total weight
            // This leaves the heaviest 25% of work in the right partition
            double targetWeight = totalWeight * 0.75;
    ...
    private class ExtensionSearchTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // TOP-HEAVY FIRST: Split at 75% weight to isolate heavy extensions
                int mid = findTopHeavySplitForExtensions(start, end);
        ...
        /**
         * Finds split point that isolates top-heavy extensions (75% weight point)
         * Extensions are ordered by PTWU, splitting at 75% prioritizes heavy items
         * ENSURES: start < result < end (strictly between)
         */
        private int findTopHeavySplitForExtensions(int start, int end) {
            // Step 1: Calculate total weight for range [start, end)
            double totalWeight = 0.0;
            for (int i = start; i < end; i++) {
        ...
        private void processExtension(int index) {
            ...
            UPUList joined = variant4.this.join(prefix, extension);
    ...
    public variant4(Map<Integer, Double> itemProfits, int k, double minPro, boolean debug, boolean debugVerbose) {
    ...
    public List<Itemset> mine(List<Transaction> rawDatabase) {
        if (DEBUG) {
            System.err.println("[DEBUG] ===== PTK-HUIM-U± Algorithm Started (VARIANT 4: Top-Heavy First) =====");
            System.err.println("[DEBUG] Task Splitting: Top-Heavy First (75% weight split to isolate heavy items)");
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 4 || args.length > 5) {
            System.err.println("Usage: variant4 <database_file> <profit_file> <k> <min_probability> [--debug | --debug-verbose]");
            System.exit(1);
        }
        ...
        variant4 algorithm = new variant4(profits, k, minPro, debug, debugVerbose);
