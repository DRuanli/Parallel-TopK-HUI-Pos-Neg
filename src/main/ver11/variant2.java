 ...
 *
 * VARIANT 2: Asymmetric Weighted Split (1:2 ratio)
 * Splits at 33% weight point for better work-stealing efficiency
 */
public class variant2 {
    ...
    // ============================================================================
    // FORKJOIN TASKS FOR PARALLEL PROCESSING (ASYMMETRIC 1:2 SPLIT)
    // ============================================================================

    /**
     * Parallel task for processing single-item prefixes
     * VARIANT 2: Uses PTWU-weighted asymmetric 1:2 splitting (33% / 67%)
     */
    private class PrefixMiningTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // ASYMMETRIC 1:2 SPLIT: Split at 33% weight point for work-stealing optimization
                int mid = findAsymmetricSplitForItems(start, end);
        ...
        /**
         * Finds asymmetric split point (1:2 ratio) based on PTWU weights
         * Returns split index where accumulated weight ≈ totalWeight / 3 (33% point)
         * This creates smaller left partition that finishes quickly for work-stealing
         * ENSURES: start < result < end (strictly between)
         */
        private int findAsymmetricSplitForItems(int start, int end) {
            ...
            // Step 2: Find split point at 33% of total weight (1:2 ratio)
            double targetWeight = totalWeight / 3.0;
    ...
    private class ExtensionSearchTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // ASYMMETRIC 1:2 SPLIT: Split at 33% weight point for work-stealing optimization
                int mid = findAsymmetricSplitForExtensions(start, end);
        ...
        /**
         * Finds asymmetric split point (1:2 ratio) based on PTWU weights of extensions
         * Returns split index where accumulated weight ≈ totalWeight / 3 (33% point)
         * ENSURES: start < result < end (strictly between)
         */
        private int findAsymmetricSplitForExtensions(int start, int end) {
            // Step 1: Calculate total weight for range [start, end)
            double totalWeight = 0.0;
            for (int i = start; i < end; i++) {
        ...
        private void processExtension(int index) {
            ...
            UPUList joined = variant2.this.join(prefix, extension);
    ...
    public variant2(Map<Integer, Double> itemProfits, int k, double minPro, boolean debug, boolean debugVerbose) {
    ...
    public List<Itemset> mine(List<Transaction> rawDatabase) {
        if (DEBUG) {
            System.err.println("[DEBUG] ===== PTK-HUIM-U± Algorithm Started (VARIANT 2: Asymmetric 1:2 Split) =====");
            System.err.println("[DEBUG] Task Splitting: PTWU-Weighted Asymmetric Split (33%/67%)");
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 4 || args.length > 5) {
            System.err.println("Usage: variant2 <database_file> <profit_file> <k> <min_probability> [--debug | --debug-verbose]");
            System.exit(1);
        }
        ...
        variant2 algorithm = new variant2(profits, k, minPro, debug, debugVerbose);
