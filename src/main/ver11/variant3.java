 ...
 *
 * VARIANT 3: Hybrid Threshold Split
 * Uses weighted split for large partitions, midpoint for small ones
 */
public class variant3 {
    ...
    // ============================================================================
    // FORKJOIN TASKS FOR PARALLEL PROCESSING (HYBRID THRESHOLD SPLIT)
    // ============================================================================

    /**
     * Parallel task for processing single-item prefixes
     * VARIANT 3: Uses hybrid approach - weighted for large, midpoint for small
     */
    private class PrefixMiningTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // HYBRID SPLIT: Use weighted for large partitions, midpoint for small
                int mid = (size > HYBRID_THRESHOLD) ?
                    findWeightedSplitForItems(start, end) :
                    start + (size / 2);
        ...
        /**
         * Finds weighted balanced split point based on PTWU weights
         * Only used for large partitions (size > HYBRID_THRESHOLD)
         * ENSURES: start < result < end (strictly between)
         */
        private int findWeightedSplitForItems(int start, int end) {
    ...
    private class ExtensionSearchTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size <= TASK_GRANULARITY) {
                ...
                // HYBRID SPLIT: Use weighted for large partitions, midpoint for small
                int mid = (size > HYBRID_THRESHOLD) ?
                    findWeightedSplitForExtensions(start, end) :
                    start + (size / 2);
        ...
        /**
         * Finds weighted balanced split point based on PTWU weights of extensions
         * Only used for large partitions (size > HYBRID_THRESHOLD)
         * ENSURES: start < result < end (strictly between)
         */
        private int findWeightedSplitForExtensions(int start, int end) {
            // Step 1: Calculate total weight for range [start, end)
            double totalWeight = 0.0;
            for (int i = start; i < end; i++) {
        ...
        private void processExtension(int index) {
            ...
            UPUList joined = variant3.this.join(prefix, extension);
    ...
    private static final int HYBRID_THRESHOLD = 100;  // Switch point for hybrid strategy
    ...
    public variant3(Map<Integer, Double> itemProfits, int k, double minPro, boolean debug, boolean debugVerbose) {
    ...
    public List<Itemset> mine(List<Transaction> rawDatabase) {
        if (DEBUG) {
            System.err.println("[DEBUG] ===== PTK-HUIM-U± Algorithm Started (VARIANT 3: Hybrid Split) =====");
            System.err.println("[DEBUG] Task Splitting: Hybrid (Weighted for size>" + HYBRID_THRESHOLD + ", Midpoint otherwise)");
    ...
    public static void main(String[] args) throws IOException {
        if (args.length < 4 || args.length > 5) {
            System.err.println("Usage: variant3 <database_file> <profit_file> <k> <min_probability> [--debug | --debug-verbose]");
            System.exit(1);
        }
        ...
        variant3 algorithm = new variant3(profits, k, minPro, debug, debugVerbose);
