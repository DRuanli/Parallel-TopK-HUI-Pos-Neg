 ...
 *
 * VERSION 8.2
 *  1. Add Batch and modify related field.
 *
 *  ERROR!!!
 *  1. It is correct all datasets except "Chess"
 *  2. Need to fix and recheck for comfirmation
 *
 * @author Elio
 * @version 8.2
 */
public class ver8_2 {
    ...
    private class ExtensionSearchTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            // Bulk branch pruning (giữ nguyên)
            if (size > 1) {
                double currentThreshold = topKManager.getThreshold();
            ...
            // THAY ĐỔI: Collect candidates for batch update
            if (size <= TASK_GRANULARITY) {
                // Collect all candidates first
                List<CandidateItemset> candidates = new ArrayList<>();

                for (int i = start; i < end; i++) {
                    CandidateItemset candidate = processExtensionForBatch(i);
                    if (candidate != null) {
                        candidates.add(candidate);
                    }
                }

                // Batch update to TopK
                if (!candidates.isEmpty()) {
                    topKManager.tryAddBatch(candidates);
                }
            } else {
                // Recursive split (giữ nguyên)
                int mid = start + (size / 2);
        ...
        /**
         * Process extension và return candidate thay vì update trực tiếp
         */
        private CandidateItemset processExtensionForBatch(int index) {
            ...
            EnhancedUtilityList joined = ver8_2.this.join(prefix, extension);
            ...
            // Return candidate instead of direct update
            if (sumEU >= currentThreshold - EPSILON &&
                joined.existentialProbability >= minPro - EPSILON) {

                // Create candidate for batch update
                CandidateItemset candidate = new CandidateItemset(
                    joined.itemset, sumEU, joined.existentialProbability
                );
            ...
            // Continue recursive search even if not a candidate
            if (index < extensions.size() - 1) {
                List<EnhancedUtilityList> newExtensions = new ArrayList<>();
                double currentThresholdForFilter = topKManager.getThreshold();
    ...
    // Constructor
    public ver8_2(Map<Integer, Double> itemProfits, int k, double minPro) {
    ...
    private void searchEnhanced(EnhancedUtilityList prefix, List<EnhancedUtilityList> extensions,
        ...
        if (viableExtensions.size() >= 10 && viableExtensions.size() < PARALLEL_THRESHOLD) {
            // Process all extensions and collect candidates
            List<CandidateItemset> batchCandidates = new ArrayList<>();

            for (int i = 0; i < viableExtensions.size(); i++) {
                EnhancedUtilityList extension = viableExtensions.get(i);
                ...
                if (sumEU >= currentThreshold - EPSILON &&
                    joined.existentialProbability >= minPro - EPSILON) {
                    batchCandidates.add(new CandidateItemset(
                        joined.itemset, sumEU, joined.existentialProbability
                    ));
                }

                // Recursive search
                if (i < viableExtensions.size() - 1) {
                    List<EnhancedUtilityList> newExtensions = new ArrayList<>();
                    double currentThresholdForFilter = topKManager.getThreshold();

                    for (int j = i + 1; j < viableExtensions.size(); j++) {
                        EnhancedUtilityList ext = viableExtensions.get(j);
                        if (ext.rtwu >= currentThresholdForFilter - EPSILON) {
                            newExtensions.add(ext);
                        } else {
                            rtwuPruned.incrementAndGet();
            ...
            if (!batchCandidates.isEmpty()) {
                topKManager.tryAddBatch(batchCandidates);
            }
        } else if (viableExtensions.size() >= PARALLEL_THRESHOLD && ForkJoinTask.inForkJoinPool()) {
    ...
    private class TopKManager {
        ...
        /**
         * Batch update method cho parallel tasks
         * Thêm method này vào TopKManager
         */
        void tryAddBatch(List<CandidateItemset> candidates) {
            // Sort candidates by utility descending
            candidates.sort((a, b) -> Double.compare(b.eu, a.eu));

            // Quick filter
            double currentThreshold = threshold;
            List<CandidateItemset> viable = candidates.stream()
                .filter(c -> c.eu >= currentThreshold - EPSILON)
                .collect(Collectors.toList());
            ...
            rwLock.writeLock().lock();
            try {
                for (CandidateItemset candidate : viable) {
                    // Inline tryAdd logic to avoid lock re-acquisition
                    if (candidate.eu < threshold - EPSILON) {
                        continue;
                    }

                    Itemset existing = topKMap.get(candidate.items);
                    if (existing != null && existing.expectedUtility >= candidate.eu - EPSILON) {
                        continue;
                    }
                    ...
                    Itemset newItemset = new Itemset(candidate.items, candidate.eu, candidate.ep);
                    topKSet.add(newItemset);
                    topKMap.put(candidate.items, newItemset);
     ...
     */
    static class CandidateItemset {
        final Set<Integer> items;
        final double eu;
        final double ep;

        CandidateItemset(Set<Integer> items, double eu, double ep) {
            this.items = items;
            this.eu = eu;
            this.ep = ep;
        }
    ...
    public static void main(String[] args) throws IOException {
        ...
        ver8_2 algorithm = new ver8_2(profits, k, minPro);
