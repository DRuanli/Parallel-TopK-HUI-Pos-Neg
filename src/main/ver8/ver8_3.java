 ...
 *
 * VERSION 8.3
 * 1. Rewrite Transaction and Utitlity List
 *
 * @author Elio
 * @version 8.3
 */
public class ver8_3 {
    protected final Map<Integer, Double> itemProfits;
    private final int k;
    protected final double minPro;
    protected static final double EPSILON = 1e-10;
    protected static final double LOG_EPSILON = -700;
    ...
    protected Map<Integer, Double> itemRTWU;
    ...
    protected final AtomicLong utilityListsCreated = new AtomicLong(0);
    ...
    static class EnhancedUtilityList {
        ...
        final List<Element> elements;              // ← KEPT for 100% compatibility
        final double rtwu;                         // RTWU value for this itemset

        // ============= OPTIMIZATION: PRE-COMPUTED AGGREGATES =============
        // The key optimization: eliminate lazy evaluation and caching
        ...
        final double existentialProbability;  // Pre-computed (unchanged from original)

        /**
         * CONSERVATIVE OPTIMIZATION CONSTRUCTOR
         * - Preserves original 'elements' storage for full compatibility
         * - Pre-computes aggregates to eliminate lazy evaluation overhead
        ...
        EnhancedUtilityList(Set<Integer> itemset, List<Element> elements, double rtwu) {
            ...
            this.elements = Collections.unmodifiableList(elements);  // ← KEEP original elements
            this.rtwu = rtwu;

            // ============= FIXED: SEPARATE CALCULATIONS =============
            // BUG FIX: Calculate sums and existential probability separately
            // The original bug was combining these with early termination
            ...
            for (Element e : elements) {
                ...
                if (e.logProbability > Math.log(1.0 - EPSILON)) {
                    ...
                    break;  // ← NOW SAFE: sums already calculated above
                } else {
                    double prob = Math.exp(e.logProbability);
        ...
        // ============= OPTIMIZED ACCESSORS (KEY IMPROVEMENT) =============

        /**
         * MAJOR OPTIMIZATION: Get sum of expected utilities - now O(1) instead of O(n)
         *
         * ORIGINAL IMPLEMENTATION:
         * - Lazy evaluation with caching
         * - Thread safety issues with Double cachedSumEU
         * - O(n) computation on first call
         *
         * OPTIMIZED IMPLEMENTATION:
         * - Pre-computed during construction
         * - No thread safety issues (immutable)
         * - Always O(1) access
         */
        double getSumEU() {
            return sumEU;  // ← Direct return, no computation or caching needed
        }

        /**
         * MAJOR OPTIMIZATION: Get sum of remaining utilities - now O(1) instead of O(n)
         *
         * ORIGINAL IMPLEMENTATION:
         * - Lazy evaluation with caching
         * - Thread safety issues with Double cachedSumRemaining
         * - O(n) computation on first call
         *
         * OPTIMIZED IMPLEMENTATION:
         * - Pre-computed during construction
        ...
        double getSumRemaining() {
            return sumRemaining;  // ← Direct return, no computation or caching needed
        }

        /**
         * Get existential probability - enhanced numerical stability
         * Same complexity as original, but more numerically stable
         */
        ...
        /**
         * COMPATIBILITY: All original element access patterns still work
         * No changes needed in existing code that uses:
         * - for (Element e : utilityList.elements)
         * - utilityList.elements.get(index)
         * - utilityList.elements.size()
         */

        /**
         * Get number of elements - O(1) (same as original)
         */
        int getSize() {
        ...
        /**
         * Check if utility list is empty - O(1) (same as original)
         */
        boolean isEmpty() {
        ...
        /**
         * Compare performance with original lazy evaluation approach
         * Use for benchmarking and validation
         */
        ...
        static class PerformanceMetrics {
            ...
            final double sumEU;
            final double sumRemaining;
            final double existentialProbability;
            final String optimizationType;

            PerformanceMetrics(int elementCount, double sumEU, double sumRemaining,
                              double existentialProbability, String optimizationType) {
            ...
            public String toString() {
                return String.format("Metrics{elements=%d, sumEU=%.2f, sumRem=%.2f, existProb=%.4f, type=%s}",
                    elementCount, sumEU, sumRemaining, existentialProbability, optimizationType);
            }
        }
        ...
        /**
         * Validate that optimized calculations match original lazy evaluation
         * Use for testing to ensure accuracy is preserved
         */
        boolean validateAgainstOriginal() {
            // Simulate original lazy calculation
            double originalSumEU = 0;
            double originalSumRemaining = 0;

            for (Element e : elements) {
                double prob = Math.exp(e.logProbability);
                originalSumEU += e.utility * prob;
                originalSumRemaining += e.remaining * prob;
            }

            // Compare with pre-computed values
            boolean sumEUMatches = Math.abs(sumEU - originalSumEU) < EPSILON;
            boolean sumRemainingMatches = Math.abs(sumRemaining - originalSumRemaining) < EPSILON;

            return sumEUMatches && sumRemainingMatches;
        }
    ...
    private class ExtensionSearchTask extends RecursiveAction {
        ...
        private void processExtension(int index) {
            ...
            EnhancedUtilityList joined = ver8_3.this.join(prefix, extension);
    ...
    // Constructor
    public ver8_3(Map<Integer, Double> itemProfits, int k, double minPro) {
    ...
    /**
     * Thread-safe Top-K Manager với ReadWriteLock optimization
     */
    private class TopKManager {
        ...
        // Thay synchronized lock bằng ReadWriteLock
        ...
        boolean tryAdd(Set<Integer> items, double eu, double ep) {
            ...
            // Optimistic check với read lock
            rwLock.readLock().lock();
            try {
                // Double-check với read lock
                if (topKSet.size() >= k && eu < threshold - EPSILON) {
                    return false;
                }

                // Check if already exists with better utility
            ...
            // Need to update - acquire write lock
            ...
            try {
                ...
                if (topKSet.size() >= k) {
                    ...
                    if (last != null) {
                        threshold = last.expectedUtility;
                    }
                }
    ...
    public static void main(String[] args) throws IOException {
        ...
        ver8_3 algorithm = new ver8_3(profits, k, minPro);
