 ...
 * 1. Minimum RTWU Safety Branch Pruning
 *
 * @author Enhanced Implementation
 * @version 3.4 - Minimum RTWU Safety Branch Pruning
 */
public class ver3_4 {
    ...
    private class ExtensionSearchTask extends RecursiveAction {
        ...
        protected void compute() {
            ...
            if (size > 1) {  // Only apply for multiple extensions
                double currentThreshold = topKManager.getThreshold();

                // Find minimum RTWU in current range
        ...
        private void processExtension(int index) {
            ...
            // Dynamic RTWU pruning (still needed for individual items)
            double currentThreshold = topKManager.getThreshold();
            ...
            EnhancedUtilityList joined = ver3_4.this.join(prefix, extension);
    ...
    public ver3_4(Map<Integer, Double> itemProfits, int k, double minPro) {
    ...
    public List<Itemset> mine(List<Transaction> rawDatabase) {
        ...
        System.out.println("=== Enhanced PTK-HUIM-UÂ± v3.3 with Bulk Branch Pruning ===");
        System.out.println("NEW: Minimum RTWU Safety Branch Pruning optimization");
        ...
        System.out.println("\nPhase 2: Enhanced parallel mining with bulk branch pruning...");
        ...
        System.out.println("  - NEW: Bulk branches pruned: " + bulkBranchPruned.get());  // NEW STATISTIC
    ...
    private void sequentialMining(List<Map.Entry<Integer, EnhancedUtilityList>> sortedItems,
        ...
        for (int i = 0; i < sortedItems.size(); i++) {
            ...
            // Mine with this prefix if extensions exist
            if (!extensions.isEmpty()) {
                searchEnhanced(ul, extensions, singleItemLists);
            }
    ...
    private Map<Integer, EnhancedUtilityList> twoPassInitialization(List<Transaction> rawDatabase) {
        ...
        for (Transaction rawTrans : rawDatabase) {
            ...
            // Build utility list elements directly without TempElement
            for (int i = 0; i < trans.items.length; i++) {
                ...
                if (logProb > LOG_EPSILON) {
                    // Calculate remaining utility immediately
                    double remaining = 0;
                    ...
                    // Get or create utility list for this item
                    EnhancedUtilityList ul = singleItemLists.computeIfAbsent(item, k -> {
                        Set<Integer> itemset = Collections.singleton(k);
                        return new EnhancedUtilityList(itemset);
                    });

                    // Add element directly
                    ul.elements.add(new EnhancedUtilityList.Element(
        ...
        // Finalize utility lists with proper calculations
        Map<Integer, EnhancedUtilityList> finalLists = new HashMap<>();
        for (Map.Entry<Integer, EnhancedUtilityList> entry : singleItemLists.entrySet()) {
            Integer item = entry.getKey();
            EnhancedUtilityList tempList = entry.getValue();

            if (!tempList.elements.isEmpty()) {
                Double rtwu = itemRTWU.get(item);
                EnhancedUtilityList finalList = new EnhancedUtilityList(
                    tempList.itemset, tempList.elements, rtwu
                );

                if (finalList.existentialProbability >= minPro - EPSILON) {
                    finalLists.put(item, finalList);
                    utilityListsCreated.incrementAndGet();
                }
            }
        }
    ...
    private void searchEnhanced(EnhancedUtilityList prefix, List<EnhancedUtilityList> extensions,
        ...
        // MINIMUM RTWU SAFETY BRANCH PRUNING - Apply before any processing
        if (extensions.size() > 1) {  // Only worthwhile for multiple extensions
            double currentThreshold = topKManager.getThreshold();

            // Find minimum RTWU efficiently
            double minRTWU = Double.MAX_VALUE;
            ...
            // If minimum RTWU is below threshold, prune entire branch
            if (minRTWU < currentThreshold - EPSILON) {
                bulkBranchPruned.incrementAndGet();
                candidatesPruned.addAndGet(extensions.size());
                return;  // Safe to prune entire branch
            }
        }
        ...
        if (extensions.size() >= PARALLEL_THRESHOLD && ForkJoinTask.inForkJoinPool()) {
            ...
            for (int i = 0; i < extensions.size(); i++) {
                ...
                // Individual RTWU pruning (still needed for dynamic threshold updates)
                if (extension.rtwu < currentThreshold - EPSILON) {
                    rtwuPruned.incrementAndGet();
                    candidatesPruned.incrementAndGet();
                    continue;
    ...
    private class TopKManager {
        ...
        private void updateThresholdLocked() {
            if (topKSet.size() >= k) {
                // Use iterator for consistent ordering instead of stream
    ...
    public static void main(String[] args) throws IOException {
        ...
        ver3_4 algorithm = new ver3_4(profits, k, minPro);
